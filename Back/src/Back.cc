...
Back::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for ( reco::TrackCollection::const_iterator track = tracks->begin();track != tracks->end(); ++track,++l){
    double energy = sqrt(track->p() * track->p() + 0.011163691);
    double trackzm =Back::InvMass(ST_RECO_Match.energy(),ST_RECO_Match.px(),ST_RECO_Match.py(),ST_RECO_Match.pz(),energy,track->px(),track->py(),track->pz());
    htrackInvMass->Fill(trackzm,weigth);
    beftrack++;
