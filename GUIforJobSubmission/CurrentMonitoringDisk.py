...
###--------------------------------- Temperature ---------------------------------------------------------
        ...
        try:
            ...
            for p in iovlist:
                ...
                for i in xrange(0,len(info),8):
                    if i+7 < len(info):
                        if int(info[i+7]) != int(subtype) and abs(int(info[i+1])) != 1: continue
            ...
            for c in detid:
                ...
                for d in c[2]:
                    if detMapName.has_key(int(d)):
                        if detMapName[int(d)].count("RE") == 0: continue
    ...
    def getDetName(self,listForName):
        ...
        if int(listForName[0]) == 0:
            ...
            detName = "RE"+str(listForName[1])+"_R"+str(listForName[4])
            if 2 <= int(listForName[2]) <= 7: detName += "_S01"
            if 8 <= int(listForName[2]) <= 13: detName += "_S02"
            if 14 <= int(listForName[2]) <= 19: detName += "_S03"
            if 20 <= int(listForName[2]) <= 25: detName += "_S04"
            if 26 <= int(listForName[2]) <= 31: detName += "_S05"
            if 32 <= int(listForName[2]) <= 36 or int(listForName[2]) == 1: detName += "_S06"

            detName += "_CH"+str(listForName[2])
...
if __name__ == '__main__':
    ...
    print root.getValues("RE-2_R3_S04_CH20")
