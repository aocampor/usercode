...
if __name__ == "__main__":
    ...
    tmpath = '/tmp/' + me[0] + '/'
    tmpath1 = casa[0] + '/scratch2/' + str(sys.argv[2]) + '/job/'
    
    os.system('mkdir ' + casa[0] + '/scratch2/' + str(sys.argv[2]))
    ...
    for i in range(numjob):
        ...
        job = tmpath1 + 'MergeTrackTemplate_' + str(sys.argv[2]) + '_' + str(sys.argv[3]) + str(i) +'.job'
        print str(i) + ' ' + job
        os.system('cp MergeTrackTemplate.job ' + job )
        ...
        for item in b[i]:
            ...
            fh.writelines('   ch2.Add(\"rfio:' + path + '/' + item + '\");\n')
            fh.writelines('   ch3.Add(\"rfio:' + path + '/' + item + '\");\n')

        fh.writelines('   ch.Merge(\"'+tmpath + 'mergeExtrapolation'+str(sys.argv[2])+ '_' +str(i) +'.root\");\n')
        fh.writelines('   ch1.Merge(\"'+tmpath + 'mergeGlobal'+str(sys.argv[2])+ '_' +str(i) +'.root\");\n')
        fh.writelines('   ch2.Merge(\"'+tmpath + 'mergeGlobalTrack'+str(sys.argv[2])+ '_' +str(i) +'.root\");\n')
        fh.writelines('   ch3.Merge(\"'+tmpath + 'mergeLocal'+str(sys.argv[2])+ '_' +str(i) +'.root\");\n')
        fh.writelines('   .! hadd '+tmpath + 'MergeTrack'+str(sys.argv[2])+'_' +str(i) +'.root '+tmpath + 'mergeExtrapolation'+str(sys.argv[2])+ '_' +str(i) +'.root '+tmpath + 'mergeGlobal'+str(sys.argv[2])+ '_' +str(i) +'.root ' + tmpath + 'mergeGlobalTrack'+str(sys.argv[2])+ '_' +str(i) +'.root ' + tmpath + 'mergeLocal'+str(sys.argv[2])+ '_' +str(i) +'.root \n')
        fh.writelines('   .! cmsStageOut '+ tmpath + 'MergeTrack'+str(sys.argv[2])+ '_' +str(i) +'.root ' + path + '/ \n')
        fh.writelines('EOF \n')
        fh.close()
