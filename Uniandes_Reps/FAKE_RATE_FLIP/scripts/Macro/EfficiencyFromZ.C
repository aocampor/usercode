...
void EfficiencyFromZ::Loop()
{
  ...
  else hfile =  new TFile("../../root_files/iso.root","RECREATE","Identification Efficiency");
  ...
  */
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if( probe_gsfEle_isEE == 1 && probe_sigmaietaieta < 0.03 && probe_dphi < 0.03 && probe_deta < 0.007 && probe_hovere < 0.025)
      vbtf80 = true;
    if( probe_gsfEle_pt > 20 && iso < 0.1 && vbtf80 && fabs(probe_gsfEle_eta) < 2.5 && probe_nmisshits <= 1 && probe_dcot < 0.02 && probe_dist < 0.02)
      elsel = true;
