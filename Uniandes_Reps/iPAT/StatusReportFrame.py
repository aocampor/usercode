...
class StatusReportFrame(Frame):
    def __init__(self, root_file_name,ds,run,parent=None):
        ...
        self.fields = 'Bx min', 'Bx max', 'Cls min','Cls max', 'Eff min', 'Eff max', 'Masked strip min', 'Masked strip max','Noise min', 'Noise max'
...
##            StatusReport(self.RootFileName,values)
            cmd = './StatusReport.py ' + str(self.RootFileName) +' '+ str(self.ds)+ ' ' + str(self.run)+ ' ' + str(values[2]) + ' ' + str(values[3]) + ' ' + str(values[4]) + ' ' + str(values[5]) + ' ' + str(values[6]) + ' ' + str(values[7]) + ' ' + str(values[8]) + ' ' + str(values[9]) + ' ' + str(values[10]) + ' ' + str(values[11]) +'&'
            os.system(cmd)
    ...
    def makeform(self,root, fields):
        entries = []
        i = 0
        default = ['-3','+3','1','10','0','100','0','100','0','1000']
        for field in fields:
