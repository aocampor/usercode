...
if __name__ == "__main__":
    ...
    numfiles_p = [item[:-1] for item in os.popen('rfdir /castor/cern.ch/cms' + path + ' | grep ' + str(sys.argv[3]) + ' | grep -v Merge  | grep -v ' + str(sys.argv[2])+' | wc | awk \'{print $1}\'')]
    
    files_p = [item[:-1] for item in os.popen('rfdir /castor/cern.ch/cms' + path + ' | grep ' + str(sys.argv[3]) +' | grep -v Merge  | grep -v ' + str(sys.argv[2])+ ' | awk \'{print $9}\'')]
    
    try:
        if (int(numfiles_p[0]) != 1 ):
            numjob = sqrt(float(numfiles_p[0])-1)
        else:
            numjob = 1
        numtemp = int(numfiles_p[0])/numjob+1    
    except:
        pass
    ...
    for i in range(int(numfiles_p[0])):
        a.append(str(files_p[i]))
        j=j+1
        if (j==int(numtemp)):
            b.append(a)
            a = []
