...
class SusyRead{
 public :
  ...
  static const double luminosityint = 100.;
...
void SusyRead::Loop(){
  ...
  for (Long64_t jentry=0; jentry<nentries; jentry++) {
    ...
    //    std::cout << "sel_jet.size " << sel_jet.size() << std::endl;
    //    std::cout << "htvec[0] " << htvec[0] << std::endl;
...
std::vector< std::vector<double> >  SusyRead::AlphaT(std::vector<std::vector< double > > Jetsp){
  ...
  for(unsigned int n1=1;n1<=Jets.size()/2;n1++){
    ...
    int jet_index[50];
    int jet2_index[50];
    for( int a=0; a<50; a++) jet_index[a] = 0;
...
SusyRead::SusyRead(TTree *tree,char *output, double cs){
  ...
  //std::cout << "Pre weight: " << weight << endl;
